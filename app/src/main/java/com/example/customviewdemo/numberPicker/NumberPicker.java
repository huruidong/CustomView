...
public class NumberPicker extends LinearLayout {
    ...
    private static final float UNSELECTED_MIN_TEXT_SIZE_SCALE = 0.3F;

    /**
     * The resource id for the default layout.
     */
    private static final int DEFAULT_LAYOUT_RESOURCE_ID = R.layout.ss_number_picker;
    ...
    private int                  mUnitPadding               = 0;
    private Paint                mPaintUnit                 = new Paint();
    private Map<String, Integer> mTextWidthCache            = new ConcurrentHashMap<>();
    private int                  mMaxWidthOfDisplayedValues = 0;

    private float mUnSelectedMinScale = UNSELECTED_MIN_TEXT_SIZE_SCALE;
    ...
    public void setUnit(String unit, int unitSize) {
        setUnitText(unit);
    }

    public void setUnitText(String UnitText) {
        if (null == UnitText || UnitText.equals(mUnitText)) {
            return;
        }
        if (-1 == mUnitTextColor) {
        ...
        if (0 == mUnitPadding) {
            mUnitPadding = (int) getResources().getDimension(
                    R.dimen.ss_numberpicker_text_unit_padding);
        }
        mPaintUnit.setColor(mUnitTextColor);
        mPaintUnit.setAntiAlias(true);
        mPaintUnit.setTextAlign(Paint.Align.CENTER);
        mPaintUnit.setTextSize(mUnitTextSize);
        mUnitText = UnitText;
        postInvalidate();
    }

    public float getmUnSelectedMinScale() {
        return mUnSelectedMinScale;
    }

    public void setmUnSelectedMinScale(float mUnSelectedMinScale) {
        this.mUnSelectedMinScale = mUnSelectedMinScale;
    }

    /**
     * 获取displayValues中最长value的宽度
     *
     * @param array
     * @param paint
     * @return
    ...
    public void setUnitTextColorByResId(int resId) {
        setUnitTextColor(getResources().getColor(resId));
    }

    public void setUnitTextSizeByResId(int resId) {
        setUnitTextSize(getResources().getDimension(resId));
    }
    ...
    public void setUnitTextSize(float size) {
        mUnitTextSize = size;
        mPaintUnit.setTextSize(size);
        postInvalidate();
    }

    public void setUnitPadding(int padding) {
    ...
    public void setTextSizeByResId(int resId) {
        setTextSize((int) getResources().getDimension(resId));
    }
    ...
    void changeOneSilently(String value, boolean isAdd) {
        if (isAdd) {
            ...
            this.changeDisplayedValue(value, true);
        } else {
            setMinValue(getMinValue() - 1);
            setMaxValue(getMaxValue() - 1);
            this.changeDisplayedValue(value, false);
        }
    ...
    private void changeDisplayedValue(String value, boolean isAdd) {
        if (value == null) {
            Log.d("SsNumberPicker", "add a null value");
        } else {
            String[] mTempDisplayedValues = getDisplayedValues();
            if (null == mTempDisplayedValues || mTempDisplayedValues.length <= 0) {
                mTempDisplayedValues = new String[1];
            }
            List<String> temp = new ArrayList<>(Arrays.asList(mTempDisplayedValues));
            if (isAdd) {
                temp.add(value);
                temp.remove(0);
            } else {
            ...
            setDisplayedValues(temp.toArray(new String[mTempDisplayedValues.length]));
        }
    }
    ...
    public NumberPicker(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        final int layoutResId = attributesArray.getResourceId(
                R.styleable.NumberPicker_ssinternalLayout, DEFAULT_LAYOUT_RESOURCE_ID);

        mHasSelectorWheel = true;

        mHideWheelUntilFocused = attributesArray.getBoolean(
                R.styleable.NumberPicker_sshideWheelUntilFocused, false);

        mSolidColor = attributesArray.getColor(R.styleable.NumberPicker_sssolidColor, 0);

        final Drawable selectionDivider = attributesArray.getDrawable(
                R.styleable.NumberPicker_ssselectionDivider);
        ...
        mSelectionDividerHeight = attributesArray.getDimensionPixelSize(
                R.styleable.NumberPicker_ssselectionDividerHeight, defSelectionDividerHeight);
        ...
        mSelectionDividersDistance = attributesArray.getDimensionPixelSize(
                R.styleable.NumberPicker_ssselectionDividersDistance,
                defSelectionDividerDistance);

        mMinHeight = attributesArray.getDimensionPixelSize(
                R.styleable.NumberPicker_ssinternalMinHeight, SIZE_UNSPECIFIED);

        mMaxHeight = attributesArray.getDimensionPixelSize(
                R.styleable.NumberPicker_ssinternalMaxHeight, SIZE_UNSPECIFIED);
        ...
        mMinWidth = attributesArray.getDimensionPixelSize(
                R.styleable.NumberPicker_ssinternalMinWidth, SIZE_UNSPECIFIED);

        mMaxWidth = attributesArray.getDimensionPixelSize(
                R.styleable.NumberPicker_ssinternalMaxWidth, SIZE_UNSPECIFIED);
        ...
        mVirtualButtonPressedDrawable = attributesArray.getDrawable(
                R.styleable.NumberPicker_ssvirtualButtonPressedDrawable);

        mUnSelectedMinScale = attributesArray.getFloat(R.styleable.NumberPicker_unSelectedMinScaleValue, UNSELECTED_MIN_TEXT_SIZE_SCALE);
        if (mUnSelectedMinScale > 1f || mUnSelectedMinScale <= 0f) {
            mUnSelectedMinScale = UNSELECTED_MIN_TEXT_SIZE_SCALE;
        }
    ...
    private void initAttrs(Context context, AttributeSet attrs) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            } else if (attr == R.styleable.NumberPicker_unitText) {
            ...
            } else if (attr == R.styleable.NumberPicker_unitPadding) {
                setUnitPadding((int) a.getDimension(attr, context.getResources().getDimension(
                        R.dimen.ss_numberpicker_text_unit_padding)));
            }
        }
        a.recycle();
    ...
    private void initSoundPool() {
        if (isNeedSound || isNeedVibrate) {
            mSoundPoolUtils = SoundPoolUtils.getInstance(getContext());
        }
        if (isNeedSound) {
            mSoundLoadId = mSoundPoolUtils.loadVideo(R.raw.numberpicker_sound_effect);
        }
    }
    ...
    public void scrollBy(int x, int y) {
        ...
        //SSUI <SSUI_Widget> <hurd> <2019-09-29> add for 修复滑动滚轮处于中间状态时，会一直调用的问题 begin
        while (mCurrentScrollOffset - mInitialScrollOffset > mSelectorTextGapHeight && y > 0) {
        ...
        //SSUI <SSUI_Widget> <hurd> <2019-09-29> add for 修复滑动滚轮处于中间状态时，会一直调用的问题 end
...
//        }
    ...
    protected void drawableStateChanged() {
        ...
        if (selectionDivider != null && selectionDivider.isStateful() && selectionDivider.setState(
                getDrawableState())) {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < selectorIndices.length; i++) {
            ...
            if ((showSelectorWheel && i != SELECTOR_MIDDLE_ITEM_INDEX) || (i == SELECTOR_MIDDLE_ITEM_INDEX && mInputText.getVisibility() != VISIBLE)) {
                ...
                float scaleValue = (1f -
                        Math.abs(
                                Math.abs(y * 1f / (SELECTOR_MIDDLE_ITEM_INDEX * mSelectorElementHeight + mInitialScrollOffset))
                                        - 1f)
                ) * (1 - mUnSelectedMinScale) + mUnSelectedMinScale;
                canvas.scale(scaleValue, scaleValue, x, y);
                canvas.drawText(scrollSelectorValue, x, y, mSelectorWheelPaint);
                canvas.restore();
            }

            if (mMaxWidthOfDisplayedValues <= 0) {
                if (null == getDisplayedValues() || getDisplayedValues().length == 0) {
                    mMaxWidthOfDisplayedValues = getTextWidth(String.valueOf(getMaxValue()),
                                                              mSelectorWheelPaint);
                } else {
                    mMaxWidthOfDisplayedValues = getMaxWidthOfTextArray(getDisplayedValues(),
                                                                        mSelectorWheelPaint);
                }
            }

            if (null != mUnitText && !"".equals(mUnitText)) {
                canvas.drawText(mUnitText, x // 已选中displayValue的中心x
                                        + mMaxWidthOfDisplayedValues / 2 // 加上displayValue宽度的一半
                                        + getTextWidth(mUnitText, mPaintUnit) / 2 + mUnitPadding,
                                (int) ((getTop() + getBottom()) / 2  // view的中心点;
                                        - getTop() // 减去设置了paddingtop的偏移量
                                        + getTextCenterYOffset(
                                        mSelectorWheelPaint.getFontMetrics())), mPaintUnit);
            }
    ...
    private boolean updateInputTextView() {
         ...
         * number.
         */
        String text = (mDisplayedValues == null) ? formatNumber(
                mValue) : mDisplayedValues[mValue - mMinValue];
        if (!TextUtils.isEmpty(text)) {
            ...
            if (!text.equals(beforeText.toString())) {
                ...
                //                    event.setSource(SsNumberPicker.this,
                //                            AccessibilityNodeProviderImpl.VIRTUAL_VIEW_ID_INPUT);
                //                    requestSendAccessibilityEvent(SsNumberPicker.this, event);
                //                }
                return true;
            }
        }
    ...
    private void notifyChange(int previous, int current) {
        if (isNeedSound() && null != mSoundPoolUtils) {
            mSoundPoolUtils.playVideoByLoad(mSoundLoadId);
        }
        ...
        public CharSequence filter(CharSequence source, int start, int end, Spanned dest, int dstart, int dend) {
            ...
            if (mDisplayedValues == null) {
                ...
                String result = String.valueOf(
                        dest.subSequence(0, dstart)) + filtered + dest.subSequence(dend,
                                                                                   dest.length());
    ...
    //                            if (SsNumberPicker.this.isEnabled()
    ...
    //                            if (SsNumberPicker.this.isEnabled() && !mInputText.isFocused()) {
    ...
    //                        case AccessibilityNodeInfo.ACTION_CLICK: {
    //                            if (SsNumberPicker.this.isEnabled()) {
    //                                SsNumberPicker.this.changeValueByOne(true);
    //                                sendAccessibilityEventForVirtualView(virtualViewId,
    ...
    //                                SsNumberPicker.this.changeValueByOne(increment);
    //                                sendAccessibilityEventForVirtualView(virtualViewId,
    //                                        AccessibilityEvent.TYPE_VIEW_CLICKED);
    //                                return true;
    //                            }
    ...
    //                mInputText.onInitializeAccessibilityEvent(event);
    //                mInputText.onPopulateAccessibilityEvent(event);
    //                event.setSource(SsNumberPicker.this, VIRTUAL_VIEW_ID_INPUT);
    //                requestSendAccessibilityEvent(SsNumberPicker.this, event);
    //            }
    //        }
    //
    ...
    //                event.setEnabled(SsNumberPicker.this.isEnabled());
    //                event.setSource(SsNumberPicker.this, virtualViewId);
    //                requestSendAccessibilityEvent(SsNumberPicker.this, event);
    //            }
    //        }
    //
    ...
    //            info.setSource(SsNumberPicker.this, VIRTUAL_VIEW_ID_INPUT);
    ...
    //            info.setSource(SsNumberPicker.this, virtualViewId);
    //            info.setParent(SsNumberPicker.this);
    //            info.setText(text);
    //            info.setClickable(true);
    //            info.setLongClickable(true);
    //            info.setEnabled(SsNumberPicker.this.isEnabled());
    //            Rect boundsInParent = mTempRect;
    //            boundsInParent.set(left, top, right, bottom);
    //            info.setVisibleToUser(isVisibleToUser(boundsInParent));
    //            info.setBoundsInParent(boundsInParent);
    ...
    //            AccessibilityNodeInfo info = AccessibilityNodeInfo.obtain();
    //            info.setClassName(SsNumberPicker.class.getName());
    //            info.setPackageName(mContext.getPackageName());
    //            info.setSource(SsNumberPicker.this);
    //
    //            if (hasVirtualDecrementButton()) {
    //                info.addChild(SsNumberPicker.this, VIRTUAL_VIEW_ID_DECREMENT);
    //            }
    //            info.addChild(SsNumberPicker.this, VIRTUAL_VIEW_ID_INPUT);
    //            if (hasVirtualIncrementButton()) {
    //                info.addChild(SsNumberPicker.this, VIRTUAL_VIEW_ID_INCREMENT);
    //            }
    //
    ...
    public static final Formatter TWO_DIGIT_FORMATTER = new Formatter() {
        ...
        final java.util.Formatter mFmt = new java.util.Formatter(mBuilder, Locale.getDefault());
