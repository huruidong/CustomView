...
public class CustomIndicator extends View {
    ...
    private final int             SPLIT_OFFSET        = DisplayUtils.dp2px(getContext(), 10);
    private final float           SPLIT_RADIUS_FACTOR = 1.4f;
    ...
    private void init(AttributeSet attrs) {
        ...
        normalPointRadius = typedArray.getDimensionPixelSize(
                R.styleable.CustomIndicator_normalPointRadius, DEFAULT_NORMAL_POINT_RADIUS);
        selectedPointRadius = typedArray.getDimensionPixelSize(
                R.styleable.CustomIndicator_selectedPointRadius,
                indicatorType == INDICATOR_TYPE_GRADUAL ? DEFAULT_NORMAL_POINT_RADIUS : DEFAULT_SELECTED_POINT_RADIUS);
        pointInterval = typedArray.getDimensionPixelSize(R.styleable.CustomIndicator_pointInterval,
                                                         20);
        normalPointColor = typedArray.getColor(R.styleable.CustomIndicator_normalPointColor,
                                               Color.parseColor("#FFFFFF"));
        selectedPointColor = typedArray.getColor(R.styleable.CustomIndicator_selectedPointColor,
        ...
        if (viewPager != null) {
            viewPager.addOnPageChangeListener(new ViewPager.OnPageChangeListener() {
                ...
                public void onPageScrolled(int position, float positionOffset, int positionOffsetPixels) {
                    Log.w("huruidong",
                          "posInfo: " + currentPagePosition + " " + position + " " + positionOffset + " " + positionOffsetPixels);
                    // 动态计算当前页与目标页位置
            ...
            if (adapter != null && adapter instanceof PagerAdapter) {
                pointCount = ((PagerAdapter) adapter).getCount();
    ...
    protected void onDraw(Canvas canvas) {
        if (pointCount <= 0) {
            Log.e("huruidong", "adapter size is 0");
            return;
        }
            ...
            for (int i = 0; i < pointCount; i++) {
                ...
                for (int k = 0; k < relativeControlPoints.size() / 2; k++) {
                    ...
                    if (i == currentPagePosition || i == targetPagePosition) {
                        ...
                        controlPointX1 = centerX + relativeControlPoints.get(
                                k * 2).x * stretchFactor;
                        controlPointY1 = centerY + relativeControlPoints.get(
                                k * 2).y * stretchFactor;
                        controlPointX2 = centerX + relativeControlPoints.get(
                                k * 2 + 1).x * stretchFactor;
                        controlPointY2 = centerY + relativeControlPoints.get(
                                k * 2 + 1).y * stretchFactor;
                    } else {
                    ...
                    arcPath.cubicTo(controlPointX1, controlPointY1, controlPointX2, controlPointY2,
                                    endX, endY);
                    if (indicatorType == INDICATOR_TYPE_GRADUAL || indicatorType == INDICATOR_TYPE_SCALE_AND_GRADUAL) {
        ...
        for (int i = 0; i < pointCount; i++) {
            ...
            if (i == currentPagePosition) {
                ...
                if (currentPagePosition == targetPagePosition) {
                    splitArcPath.moveTo(
                            centerX + selectedSplitPointCenterXOffset + selectedSplitPointRadius,
                            centerY);
                } else if (currentPagePosition > targetPagePosition) {
                    splitArcPath.moveTo(
                            centerX + selectedSplitPointCenterXOffset + selectedSplitPointRadius + splitOffset,
                            centerY);
                } else {
                    float currentX = centerX + selectedSplitPointCenterXOffset + selectedSplitPointRadius;
                    // 根据粘合偏移量控制分裂圆形的起点(初始滑动分裂阶段为零，后半段粘合时有效)
                    splitArcPath.moveTo(currentX + getCurrentBondingOffset(currentX - centerX),
                                        centerY);
                    // 根据滑动分裂偏移量调整当前圆形的起点
            ...
            for (int k = 0; k < relativeControlPoints.size() / 2; k++) {
                    ...
                    case 1: {
                        ...
                        if (i == currentPagePosition) {
                            ...
                            if (currentPagePosition != targetPagePosition) {
                                ...
                                if (currentPagePosition > targetPagePosition) {
                                    endX -= offset;
                                    selectedSplitEndX -= getCurrentBondingOffset(
                                            centerX - selectedSplitEndX);
                                } else {
                ...
                if (i == currentPagePosition) {
                    ...
                    controlPointX1 = centerX + selectedSplitPointCenterXOffset + relativeControlPoints.get(
                            k * 2).x * stretchFactor;
                    controlPointY1 = centerY + relativeControlPoints.get(k * 2).y * stretchFactor;
                    controlPointX2 = centerX + selectedSplitPointCenterXOffset + relativeControlPoints.get(
                            k * 2 + 1).x * stretchFactor;
                    controlPointY2 = centerY + relativeControlPoints.get(
                            k * 2 + 1).y * stretchFactor;
                    splitArcPath.cubicTo(controlPointX1, controlPointY1, controlPointX2,
                                         controlPointY2, selectedSplitEndX, selectedSplitEndY);
                    normalPointPaint.setColor(selectedPointColor);
