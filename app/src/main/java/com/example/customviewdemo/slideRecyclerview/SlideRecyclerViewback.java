 ...
 */
public class SlideRecyclerViewback extends RecyclerView {
    ...
    public SlideRecyclerViewback(Context context) {
        super(context);
        init();
    }

    public SlideRecyclerViewback(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        init();
    }

    public SlideRecyclerViewback(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    public boolean onInterceptTouchEvent(MotionEvent e) {
        ...
        switch (e.getAction()){
            case MotionEvent.ACTION_DOWN:
                ...
                if (mLastMoveView != null && mLastMoveView != mMoveView && mLastMoveView.getScrollX() != 0){
                    closeMenu();
                }
        ...
        switch (e.getAction()){
            ...
            case MotionEvent.ACTION_CANCEL:
                if (mMoving) {
                    ...
                    if (!mScroller.isFinished()){
                        mScroller.abortAnimation();
                        mLastMoveView.scrollTo(mScroller.getFinalX(), 0);
                    }
                    mMoving = false;
                    ...
                    mVelocity.computeCurrentVelocity(1000);
                    int scrollX = mLastMoveView.getScrollX();
                    ...
                    invalidate();
                } else if (mLastMoveView != null && mLastMoveView.getScrollX() != 0){
                    //若不是水平滑动状态，菜单栏开着则关闭
                    closeMenu();
                }
                releaseVelocity();
    ...
    public void computeScroll() {
        if (mScroller.computeScrollOffset()) {
            if (isInWindow(mLastSelectedItemView)){
                mLastMoveView.scrollTo(mScroller.getCurrX(), 0);
                invalidate();
            }else {
    ...
    public void closeMenu(){
        mScroller.startScroll(mLastMoveView.getScrollX(), 0, -mLastMoveView.getScrollX(), 0 , 500);
        invalidate();
    }
    ...
    public void closeMenuNow(){
        if (mLastMoveView != null && mLastMoveView.getScrollX() != 0) {
            mLastMoveView.scrollTo(0, 0);
        }
    }
