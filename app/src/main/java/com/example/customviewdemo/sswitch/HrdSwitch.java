 ...
 */
public class HrdSwitch extends CompoundButton {
     ...
     */
    public HrdSwitch(Context context) {
        this(context, null);
    }
     ...
     */
    public HrdSwitch(Context context, AttributeSet attrs) {
     ...
     */
    public HrdSwitch(Context context, AttributeSet attrs, int defStyleAttr) {
     ...
     */
    public HrdSwitch(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
...
//            mThumbDrawable = getContext().getDrawable(R.drawable.hrd_switch_thumb_default);
//        }
...
//            mTrackDrawable = getContext().getDrawable(R.drawable.hrd_switch_track_default);
//        }
        if (null != mTrackDrawable) {
        ...
        mUseFallbackLineSpacing = context.getApplicationInfo().targetSdkVersion >= 28;
    ...
    public void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mShowText) {
            maxTextWidth = Math.max(mOnLayout.getWidth(),
                                    mOffLayout.getWidth()) + mThumbTextPadding * 2;
        } else {
            maxTextWidth = 0;
        }
        ...
        Log.d("huruidong", "at ssui at com ---> onMeasure() "
                + "\nmSwitchMinWidth: " + mSwitchMinWidth
                + "\n2*mThumbWidth: " + (int) (2 * mThumbWidth)
                + "\n1.5*mThumbWidth: " + (int) (1.5f * mThumbWidth)
        );
        final int switchWidth = Math.max(mSwitchMinWidth,
                                         (int) (1.65f * mThumbWidth) + paddingLeft + paddingRight);
...
//        super.onPopulateAccessibilityEventInternal(event);
    ...
    private Layout makeLayout(CharSequence text) {
        final CharSequence transformed = (mSwitchTransformationMethod != null) ? mSwitchTransformationMethod.getTransformation(
                text, this) : text;

        int width = (int) Math.ceil(
                Layout.getDesiredWidth(transformed, 0, transformed.length(), mTextPaint,
                                       getTextDirectionHeuristic()));
        //        return StaticLayout.Builder.obtain(transformed, 0, transformed.length(), mTextPaint,
        //                                           width).setUseLineSpacingFromFallbacks(
        //                mUseFallbackLineSpacing).build();
        return new StaticLayout(transformed, mTextPaint, width, Layout.Alignment.ALIGN_NORMAL, 1.f,
                                0, true);
    }
            ...
            case MotionEvent.ACTION_MOVE: {
                switch (mTouchMode) {
                    ...
                    case TOUCH_MODE_DOWN: {
                        final float x = ev.getX();
                        final float y = ev.getY();
                        if (Math.abs(x - mTouchX) > mTouchSlop || Math.abs(
    ...
    public CharSequence getAccessibilityClassName() {
        return HrdSwitch.class.getName();
    }
...
//        super.onInitializeAccessibilityNodeInfoInternal(info);
    ...
    private static final FloatProperty<HrdSwitch> THUMB_POS = new FloatProperty<HrdSwitch>(
            "thumbPos") {
        @Override
        public Float get(HrdSwitch object) {
            return object.mThumbPosition;
        }

        @Override
        public void setValue(HrdSwitch object, float value) {
